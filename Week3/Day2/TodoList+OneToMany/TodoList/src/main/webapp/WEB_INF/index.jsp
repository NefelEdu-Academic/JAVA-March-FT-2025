...
<c:out value="${todo.user.name}"></c:out>

</td>
<td>
<a href="/todo/edit/${todo.id}/${todo.title}" class="btn btn-success">Edit</a>
...
<a href="/user/new" class="btn btn-success">Create UIser</a>
<!-- Test if empty todo -->
<c:out value="${users.size()}"></c:out>
       <c:if test="${users.size() == 0}"> 
       <div class="alert alert-warning" role="alert"> There are no users yet. </div> 
       </c:if>
...
<td>
 <!--<c:out value="${user.myTodos}"></c:out>-->
 <p > <c:out value="${user.myTodos.size()}"></c:out></p>
